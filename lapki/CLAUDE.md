...
- **"Прилаштування", не "усиновлення"** — у всьому user-facing тексті (UI теми/адмінки, email-листи, повідомлення про помилки) використовується слово **"прилаштування"** ("заявка на прилаштування", "тварина прилаштована"), а не "усиновлення". Тема (`wp-content/themes/lapki/`) уже послідовно дотримується цього; у плагіні (особливо `class-lapki-rest-api.php`, `class-lapki-migrations.php`, шаблони email-повідомлень) є легасі-місця зі старим словом "усиновлення" — не копіювати їх як зразок у новому коді, і за нагоди виправляти на "прилаштування".
- Технічні ідентифікатори (назви таблиць `wp_lapki_applications`, REST-маршрут `/applications`, назви класів/методів на кшталт `Lapki_Application`, `send_application_emails()`) **не перейменовуються** — це стосується лише тексту, видимого користувачу.
- **Ім'я завжди йде першим, прізвище — другим** — у будь-якій формі з обома полями спершу поле "Ім'я", тоді "Прізвище" (сесія 115, 2026-09-11: порядок змінено з прізвище→ім'я на ім'я→прізвище у `templates/shortcode-signup.php` і `templates/edit-profile.php` — усі форми плагіна з цією парою полів). Технічні назви полів (`first_name`, `last_name`) і порядок аргументів у коді (`$first_name, $last_name` тощо) не зачіпає — це лише про порядок відображення в UI.
...
│   ├── profile.php                    # /profile/ — кабінет користувача (повний шаблон, не шорткод)
...
**Ключові поля:**
- `id` (int) - PRIMARY KEY
- `wp_user_id` (int) - **Legacy**: хто створив організацію (історичне значення). Реальна прив'язка користувачів — через `wp_lapki_organization_members` (many-to-many), не через цю колонку — див. `Lapki_Organization_Member` нижче
...
#### 2b. `wp_lapki_organization_members` - Членство користувачів в організаціях ⭐ NEW
Many-to-many: одна організація — багато користувачів (власник + учасники); один користувач — не більш ніж одна організація одночасно (`UNIQUE` на `wp_user_id`). Замінює стару модель 1-до-1.
- `id`, `organization_id`, `wp_user_id`
- `role` (varchar) - `owner` (може редагувати профіль організації) або `member` (лише тварини)
- `created_at`
...
- `get_by_wp_user_id($wp_user_id)`, `belongs_to_user($organization_id, $wp_user_id)` - тепер делегують у `Lapki_Organization_Member` (див. нижче); сигнатури не змінились

#### Lapki_Organization_Member ⭐ NEW
Членство користувачів в організаціях (many-to-many, `wp_lapki_organization_members`).
- `get_by_user($wp_user_id)` - Членство поточного користувача (з даними організації) або `null`
- `get_role($organization_id, $wp_user_id)` - `'owner'`/`'member'`/`null`
- `get_members($organization_id)` - Усі учасники організації (з `display_name`/`user_email`)
- `join($organization_id, $wp_user_id, $role)` - Приєднати (повертає `false`, якщо вже є членство деінде)
- `leave($wp_user_id)` - Прибрати з організації
...
**POST** `/wp-json/lapki/v1/organizations` ⭐ CHANGED
- Самостійна реєстрація організації — будь-який залогінений користувач (`is_user_logged_in()`, не capability). `409 already_member`, якщо вже прив'язаний до організації. Створює членство `owner` + підвищує WP-роль до `lapki_shelter_admin`

**PUT** `/wp-json/lapki/v1/organizations/{id}`
- Оновити організацію — саме **власник** (`role='owner'` у членстві), не будь-який учасник, або адмін

**DELETE** `/wp-json/lapki/v1/organizations/{id}`
- Видалити організацію (власник або адмін)

**POST** `/wp-json/lapki/v1/organizations/{id}/join` ⭐ NEW
- Приєднатися до вже існуючої організації як `member` (роль `lapki_volunteer`). `409`, якщо вже є членство

**POST** `/wp-json/lapki/v1/organizations/leave` ⭐ NEW
- Вийти зі своєї організації (власник чи учасник). `404`, якщо немає членства
...
#### Signup Endpoint (публічна реєстрація) ⭐ CHANGED — тепер лише акаунт, без організації

**POST** `/wp-json/lapki/v1/signup`
- Публічний. Обов'язкові поля: `name`, `email`, `password` (мін. 6 символів); необов'язково `phone`
- Валідує email/пароль, перевіряє унікальність email (`email_exists()`), генерує `user_login` з локальної частини email (дедуплікація через `username_exists()`)
- Створює **тільки** WP-користувача (`wp_insert_user`, роль `lapki_volunteer` за замовчуванням) — **більше не створює організацію**. Прив'язка до притулку/ГО — окремий крок у кабінеті (`POST /organizations` або `POST /organizations/{id}/join`, див. вище)
- Автологін (`wp_set_auth_cookie`), відповідь містить `redirect` → `/profile/`
...
**Меню в адмінці:**
- `Lapki → Тварини` (таблиця + пошук/пагінація через AJAX/JS — легасі-патерн, не `WP_List_Table`, див. конвенцію нижче)
- `Lapki → Додати тварину` (форма створення/редагування — готова: Leaflet карта для координат, Dropzone для фото)
- `Lapki → Організації` (таблиця + модалка створення/редагування через AJAX/JS — той самий легасі-патерн)
- `Lapki → Атрибути` (CRUD-редактор атрибутів/перекладів через AJAX/JS: фільтри, таблиця, модалка, пагінація — легасі-патерн)
- `Lapki → Email-шаблони` ⭐ NEW (сесія 108/109) — перша сторінка на справжньому `WP_List_Table` (`Lapki_Email_Templates_List_Table`), див. конвенцію нижче
- `Lapki → Статистика`
- `Lapki → Налаштування` (дефолтна дистанція пошуку, розмір сторінки, email для нотифікацій — Settings API)

#### ⚠️ Конвенція: нові таблиці в адмінці — тільки через `WP_List_Table`

Ухвалено в сесії 109 (2026-09-01): **усі нові списки/таблиці в адмінці Lapki реалізувати через нативний `WP_List_Table`** (`extends WP_List_Table`, серверний рендер даних напряму з БД у `prepare_items()`, без окремого AJAX GET на завантаження списку) — а не AJAX/JS-рендер таблиці, як історично зроблено для Тварин/Організацій/Атрибутів (`initXList()` + `loadX()`/`renderXTable()` у `js/lapki-admin.js`, що будували `<tr>` клієнтським JS з даних REST API).

Еталонна реалізація: `Lapki_Email_Templates_List_Table` (`class-lapki-admin.php`, оголошено одразу після підключення `class-wp-list-table.php`) + `Lapki_Admin::email_templates_page()`. Ключові моменти патерну:
- Первинна колонка (напр. "Призначення") використовує `$this->row_actions([...])` — WordPress-нативне посилання-дію ("Редагувати" тощо), приховане й показується при наведенні на рядок стандартним адмін-CSS ядра, без власного CSS/JS для hover-ефекту.
- Дані для модалки редагування кладуться напряму в `data-*` атрибути посилання дії (id/subject/body/...), щоб відкриття модалки не тягнуло додатковий REST-запит — JS лише читає `$(this).data(...)`.
- Збереження (create/update) лишається через REST API (`apiRequest()` у `js/lapki-admin.js`) як і раніше; після успіху — `window.location.reload()`, щоб серверний `WP_List_Table` показав актуальні дані (простіше й надійніше за клієнтський ре-рендер рядка).
- Існуючі AJAX-таблиці (Тварини/Організації/Атрибути) **не обов'язково переписувати заднім числом** під цю конвенцію, поки користувач явно не попросить — конвенція стосується нових таблиць.

**⚠️ Уточнення (сесія 109, за проханням користувача): "все на фронтенді — виключно через REST API; на бекенді прямі запити до БД/моделі — лише якщо REST API неможливий".** Це стосується й серверного рендеру `WP_List_Table`: `prepare_items()` **не** викликає модель напряму (`Lapki_Email_Template::get_all()`), а диспетчеризує **той самий** REST-контролер у процесі через `rest_do_request()` (без реального HTTP-запиту, без зайвого round-trip, але з тим самим permission callback і тією самою логікою, що й у `GET /email-templates`). Тобто РЕАЛЬНО пряме звернення до `$wpdb`/моделі лишається лише всередині самого REST-контролера (`class-lapki-rest-api.php`) — це єдина точка доступу до даних для всього плагіна, і серверний, і клієнтський код ходять тільки через неї. Той самий підхід (`rest_do_request()` замість прямого виклику моделі) застосовувати і в майбутніх `WP_List_Table`-сторінках.
...
**Ролі:** `lapki_shelter_admin` (тварини + організація), `lapki_volunteer` (тільки тварини). Адміністратор сайту отримує всі три capability автоматично при активації/апгрейді. Роль тепер визначається **дією**, не типом при реєстрації: створив організацію (`POST /organizations`) → `lapki_shelter_admin`; приєднався до чужої (`POST /organizations/{id}/join`) → `lapki_volunteer`.

`Lapki_Roles::user_owns_organization($organization_id, $wp_user_id)` ⭐ CHANGED — перевірка членства (будь-яка роль — `owner` або `member`) через `Lapki_Organization_Member`, або `manage_options`; дозволяє керувати тваринами/медіа/заявками. `Lapki_Roles::user_is_organization_owner(...)` ⭐ NEW — суворіша перевірка (лише `role='owner'`), для редагування профілю самої організації.
...
**Шорткоди:**
- `[lapki_signup]` (`render_signup_shortcode()` → `templates/shortcode-signup.php`) — публічна форма реєстрації **лише акаунта**, розміщена на сторінці `/signup/`. Поля: ім'я, email, пароль, телефон (без типу й назви організації — це окремий крок, див. `/profile/` нижче). Сабміт іде через JS (`assets/js/lapki.js::initSignupForm()`) на `POST /wp-json/lapki/v1/signup`. **Тимчасово (на прохання)** форма показується навіть залогіненим користувачам — позначено коментарем `ТИМЧАСОВО` в шаблоні для легкого відкату.

**`/profile/`** (`templates/profile.php`) ⭐ CHANGED — особистий кабінет залогіненого користувача. **Не шорткод** (раніше був `[lapki_cabinet]` на звичайній WP-сторінці `/cabinet/` — перейменовано й переведено на повний `Lapki_Frontend`-route, бо `page.php` теми обгортає шорткод-контент у вузьку `col-lg-8`, а кабінету потрібна повна ширина сайту, як на `/animals/`). Шапка профілю (аватар-круг з першою літерою імені), бокове меню з іконками:
  - **Головна** (`?tab=home`, дефолт) — ім'я/email/телефон користувача; далі залежно від членства (`Lapki_Organization_Member::get_by_user()`):
    - **є членство** — картка організації (назва, тип, місто, контакти, бейдж верифікації, бейдж ролі Власник/Учасник, посилання на публічну сторінку) + кнопка «Залишити організацію» (`POST /organizations/leave`)
    - **немає членства** — панель прив'язки з двома вкладками: «Приєднатися» (пошук + список усіх організацій, кнопка `POST /organizations/{id}/join` на кожній) і «Зареєструвати нову» (форма тип/назва/місто/телефон/email → `POST /organizations`, стає власником)
  - **Мої тварини** (`?tab=animals`) — усі тварини організації користувача, фото + бейдж статусу, клік → публічна сторінка тварини. **Лише перегляд** — форми додавання/редагування тварини з кабінету ще немає (окрема задача, див. Roadmap)
  - **Вихід** — `wp_logout_url()`
  - Незалогінених користувачів перенаправляє на посилання увійти/зареєструватись замість кабінету
  - Посилання «Профіль» у шапці теми (`header.php`) показується лише залогіненим; `POST /signup` після успіху редиректить сюди
  - JS: `assets/js/lapki.js::initCabinetOrgAttach()` — перемикач вкладок приєднання/створення, live-пошук по списку організацій, `fetch()` на join/create/leave з перезавантаженням сторінки після успіху
...
#### Що зроблено (2026-07-12, сесія 17)
- [x] **Реєстрація й прив'язка до організації розділені** — `[lapki_signup]` тепер створює лише акаунт (без типу/організації); `[lapki_cabinet]` отримав панель прив'язки (приєднатись до існуючої зі списку АБО зареєструвати нову)
- [x] **Many-to-many користувач↔організація** — нова таблиця `wp_lapki_organization_members` (`role`: owner/member), замінює стару 1-до-1 схему (`unique_wp_user` знято, backfill існуючих 5 організацій в owner-членство виконано автоматично)
- [x] **REST:** `POST /organizations` тепер self-service для будь-кого залогіненого; нові `POST /organizations/{id}/join`, `POST /organizations/leave`
- [x] Повний E2E-тест через `wp eval-file` (20 перевірок, включно з регресією на реальному продакшн-власнику) — один баг знайдено й виправлено (join не знижував роль з shelter_admin до volunteer)

#### Що зроблено (2026-07-12, сесія 18)
- [x] **`/cabinet/` → `/profile/`, повноширинний шаблон замість шорткоду** — `[lapki_cabinet]` на звичайній WP-сторінці замінено на `Lapki_Frontend`-route `templates/profile.php` (той самий патерн, що й `/animals/`, `/organizations/`) — вирішує обмеження `col-lg-8` з `page.php` теми; додано шапку профілю (аватар-круг) + іконки в боковому меню
...
- [x] Публічна frontend-форма самостійного додавання/редагування тварини (сесія 45, 2026-08-16) — `/add-animal/` тепер підтримує й редагування (`?id=N`, лише власник/учасник організації тварини — перевірка як на REST `PUT /animals/{id}`), з кнопкою "Редагувати" на `/animals/{id}/`; управління фото (видалення/головне) працює прямо з форми. Видалення тварини з фронтенду досі немає (лише через адмінку)
- [ ] Backup/restore БД
...
**Востаннє оновлена:** 2026-07-12 (сесія 18 — `/cabinet/` → `/profile/`, повноширинний шаблон замість шорткоду)
**Версія документації:** 1.5
